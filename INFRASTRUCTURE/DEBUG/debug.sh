...
docker_image_reg_name_deploy=$(jq -e '."deploy"' INFRASTRUCTURE/descriptor.json | tr -d \")
...
docker_image_reg_name_build=$(jq -e '."build"' INFRASTRUCTURE/descriptor.json | tr -d \")
...
docker run -ti -d -v "$(pwd)":/work --network=my-net ${docker_image_reg_name_build}
